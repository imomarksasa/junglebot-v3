  ...
  var invites = await message.guild.fetchInvites();
    ...
    invites.forEach(i => {
      ...
      if (i.uses === 10) {//يمديك تعدل رقم وصول العدد حق الانفايت الى اأقل أو أكثر
          message.member.addRole(message.member.guild.roles.find("name","Inviter I"))//هنآ أسم ألرتبه اللي تجيهه
.catch(RebeL =>{
console.log('`Error`: ' + RebeL);
});
}
if (i.uses === 15) {
message.member.addRole(message.member.guild.roles.find("name","Inviter II"))
.catch(RebeL =>{
console.log('`Error`: ' + RebeL);
});
}
